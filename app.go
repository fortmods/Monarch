...
var launcherProc *exec.Cmd
var antiCheatProc *exec.Cmd
var fortniteProc *exec.Cmd
...
func (a *App) Run(username string) {
...
func launch(installLocation string, antiCheatService string, args []string) uint32 {
	...
	fortniteProc = exec.Command(fortnitePath, args...)
	fortniteStartErr := fortniteProc.Start()
	...
	return uint32(fortniteProc.Process.Pid)
}
...
func closeProcs() {
	if launcherProc != nil {
		launcherProc.Process.Kill()
	}

	if antiCheatProc != nil {
		antiCheatProc.Process.Kill()
	}

	if fortniteProc != nil {
		fortniteProc.Process.Kill()
	}
